...
class SocialLinkFormatter
{
    ...
    private static array $patterns = [
        ...
        'twitter' => 'https://x.com/{username}', // Alias for backward compatibility
     ...
     */
    public static function generateProfileUrl(string $platform, ?string $username, ?string $customLink = null): string
    {
        ...
        // If no username provided, return placeholder
        if (!$username) {
            return '#';
        }
        ...
        return str_replace('{username}', $username, $pattern);
    }
    ...
    public static function getAvailablePlatforms(): array
    {
        return [
            ...
            [
                'value' => 'website',
                'label' => 'Website',
                'icon' => '<svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-twitter-x w-5 h-5" viewBox="0 0 16 16"><path d="M12.6 0.75h2.454l-5.36 6.142L16 15.25h-4.937l-3.867 -5.07 -4.425 5.07H0.316l5.733 -6.57L0 0.75h5.063l3.495 4.633L12.601 0.75Zm-0.86 13.028h1.36L4.323 2.145H2.865z" stroke-width="1"></path></svg>'
            ]
        ];
    }
    ...
    /**
     * Convert old format social media data to new format
     */
    public static function convertOldFormat(array $oldData): array
    {
        $newData = [];
        
        foreach ($oldData as $platform => $url) {
    ...
    /**
     * Convert new format social media data to old format for backward compatibility
     */
    public static function convertToOldFormat(array $newData): array
    {
        $oldData = [];
        
        foreach ($newData as $item) {
            if (!empty($item['platform'])) {
                $url = $item['custom_link'] ?? self::generateProfileUrl(
                    $item['platform'], 
                    $item['username'] ?? null
                );
                
                if ($url && $url !== '#') {
                    $oldData[$item['platform']] = $url;
                }
            }
        }
    ...
    /**
     * Alias for convertToOldFormat for backward compatibility
     */
    public static function convertNewFormatToOld(array $newData): array
    {
        return self::convertToOldFormat($newData);
    }
    ...
    public static function getPlatformIcon(string $platform): string
    {
        ...
        return '<svg class="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/></svg>';
    }
}