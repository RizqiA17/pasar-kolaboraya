...
use App\Models\RegistrationKey as RegistrationKeyModel;
use Illuminate\Support\Facades\Auth;
use Livewire\Component;
use Livewire\WithPagination;
use Livewire\Attributes\Layout;

#[Layout('layouts.admin', ['title' => 'Kelola Kode Registrasi'])]
class RegistrationKey extends Component
{
    use WithPagination;
    ...
    {
        $key = RegistrationKeyModel::findOrFail($keyId);
        $this->editingKey = $key;
        $this->key = $key->key;
    ...
    {
        $this->key = RegistrationKeyModel::generateUniqueKey();
    }
    ...
    public function createKey()
    {
        ...
        RegistrationKeyModel::create([
            'key' => $this->key ?: RegistrationKeyModel::generateUniqueKey(),
    ...
    {
        $query = RegistrationKeyModel::with('creator');
    ...
    {
        return [
            'total' => RegistrationKeyModel::count(),
            'active' => RegistrationKeyModel::where('is_active', true)->count(),
            'partisipan' => RegistrationKeyModel::where('user_type', 'partisipan')->count(),
            'tamu' => RegistrationKeyModel::where('user_type', 'tamu')->count(),
            'komunitas' => RegistrationKeyModel::where('user_type', 'komunitas')->count(),
        ];
    }

    public function render()
    {
        return view('livewire.admin.registration-key', [
            'keys' => $this->keys,
            'stats' => $this->stats,
        ]);
    }
