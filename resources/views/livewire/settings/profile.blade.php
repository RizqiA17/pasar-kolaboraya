                    ...
                    <div class="flex-1 min-w-0 mt-18">
                        ...
                        @if(auth()->user()->profile?->social_media)
                            <div class="mt-4 flex flex-wrap gap-3">
                                @foreach(json_decode(auth()->user()->profile->social_media) as $platform => $url)
                                    <a href="{{ $url }}" target="_blank" rel="noopener noreferrer" 
                                        ...
                                        @switch(strtolower($platform))
                                            ...
                                            @case('twitter')
                                                ...
                                                @break
                                            @case('github')
                                                <svg class="h-5 w-5 mr-1.5 text-gray-900" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1 space-y-8">
                ...
                <div class="group relative">
                    <div class="absolute -inset-0.5 bg-gradient-to-r from-[#379eff]/50 to-blue-500/50 rounded-xl opacity-50 group-hover:opacity-75 blur transition duration-1000 group-hover:duration-200"></div>
                    <div class="relative bg-white rounded-xl shadow-sm overflow-hidden">
                        <div class="p-6">
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center gap-2">
                                    <svg class="h-5 w-5 text-[#379eff]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @if(auth()->user()->profile?->skills)
                                    <span class="inline-flex items-center rounded-full bg-[#379eff]/10 px-2.5 py-1 text-xs font-medium text-[#379eff]">
                                        {{ count(explode(',', auth()->user()->profile->skills)) }} skills
                                    </span>
                                @endif
                            </div>
                            <div class="flex flex-wrap gap-2">
                                @foreach(explode(',', auth()->user()->profile?->skills ?? '') as $skill)
                                    @if(trim($skill))
                                        <span class="group/item inline-flex items-center px-3 py-1.5 rounded-lg text-sm font-medium bg-[#379eff]/5 text-[#379eff] ring-1 ring-inset ring-[#379eff]/10 transition-all duration-200 hover:bg-[#379eff]/10">
                                            {{ trim($skill) }}
                                        </span>
                                    @endif
                                @endforeach
                ...
                <!-- Interests Section -->
                <div class="group relative">
                    <div class="absolute -inset-0.5 bg-gradient-to-r from-emerald-500/50 to-green-500/50 rounded-xl opacity-50 group-hover:opacity-75 blur transition duration-1000 group-hover:duration-200"></div>
                    <div class="relative bg-white rounded-xl shadow-sm overflow-hidden">
                        <div class="p-6">
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center gap-2">
                                ...
                                @if(auth()->user()->profile?->interests)
                                    <span class="inline-flex items-center rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-medium text-emerald-700">
                                        {{ count(explode(',', auth()->user()->profile->interests)) }} interests
                                    </span>
                                @endif
                            </div>
                            <div class="flex flex-wrap gap-2">
                                @foreach(explode(',', auth()->user()->profile?->interests ?? '') as $interest)
                                    @if(trim($interest))
                                        <span class="group/item inline-flex items-center px-3 py-1.5 rounded-lg text-sm font-medium bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20 transition-all duration-200 hover:bg-emerald-100">
                                            {{ trim($interest) }}
                                        </span>
                                    @endif
                                @endforeach
                        ...
                        <div class="p-6">
                            <div class="flex items-center justify-between mb-6">
                                ...
                                @if(auth()->user()->profile?->contributions)
                                    <span class="inline-flex items-center rounded-full bg-[#379eff]/10 px-2.5 py-1 text-xs font-medium text-[#379eff]">
                                        {{ count(explode("\n", auth()->user()->profile->contributions)) }} contributions
                                    </span>
                                @endif
                            </div>
                            <div class="flow-root">
                                <ul role="list" class="-mb-8">
                                    @if(auth()->user()->profile?->contributions)
                                    @foreach(explode("\n", auth()->user()->profile->contributions) as $contribution)
                                        @if(trim($contribution))
                                            <li>
                                                <div class="relative pb-8">
                                                    @if(!$loop->last)
                                                        <span class="absolute top-4 left-4 -ml-px h-full w-0.5 bg-[#379eff]/20" aria-hidden="true"></span>
                                                    @endif
                                                    <div class="relative flex space-x-3">
                                                        <div>
                                                            <span class="h-8 w-8 rounded-full bg-[#379eff]/10 flex items-center justify-center ring-8 ring-white">
                                                                <svg class="h-4 w-4 text-[#379eff]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path>
                                                                </svg>
                                                            </span>
                                                        </div>
                                                        <div class="min-w-0 flex-1">
                                                            <div class="text-sm text-gray-600">{{ trim($contribution) }}</div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </li>
                                    ...
                                    <li>
                                        <div class="relative pb-8">
                                            <div class="relative flex space-x-3">
                                                ...
                                                <div class="min-w-0 flex-1">
                                                    <div class="text-sm font-medium text-purple-600 mb-1">Visi & Misi</div>
                                                    <div class="text-sm text-gray-600">{{ auth()->user()->profile->vision }}</div>
                                                </div>
                                            </div>
                                        </div>
                                    </li>
