    ...
    @else
        ...
        <div class="flex flex-col gap-6">
            ...
            <div x-data="{
                ...
                updateOrgName() {
                    // Jika berubah menjadi individu → set otomatis ke 'Individu'
                    if (this.organizationType === 'individu') {
                        this.organizationName = 'Individu';
                    }
                    // Jika dari individu ke organisasi/komunitas → kosongkan hanya jika sebelumnya individu
                    else if (this.prevType === 'individu') {
                        this.organizationName = '';
                    }
            
                    this.prevType = this.organizationType;
                    this.syncToLivewire();
                },
            }" x-init="$watch('organizationName', (value) => {
                if (organizationType !== 'individu' && value !== prevName) {
                    prevName = value;
                    syncToLivewire();
                }
            });
            $watch('organizationType', (value) => {
                if (value !== prevType) {
                    updateOrgName();
                }
            });">
                <flux:select x-model="organizationType" name="organization_type" label="Tipe Organisasi" required
                    placeholder="Pilih tipe organisasi" class="mb-6">
                ...
                <flux:input x-model="organizationName" name="organization_name"
                    x-bind:disabled="organizationType === 'individu' || !organizationType"
                    x-bind:readonly="organizationType === 'individu'" label="Nama Organisasi/Komunitas" type="text"
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                window.addEventListener('organization-updated', (e) => {
