<div
    class="bg-gradient-to-br from-blue-50/20 via-green-50/20 to-purple-50/20 dark:from-slate-900/20 dark:via-slate-800/20 dark:to-slate-900/20 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div
            ...
            @if ($search || $statusFilter !== 'all' || $roleFilter !== 'all')
                <div class="mt-4">
                    <button wire:click="$set('search', ''); $set('statusFilter', 'all'); $set('roleFilter', 'all')"
                            ...
                            <div
                                ...
                                <div class="flex items-center space-x-4 mb-3 sm:mb-0">
                                    ...
                                    <div
                                        ...
                                        @elseif($member->pivot->status === 'pending_approval') bg-orange-100 dark:bg-orange-900/30
                                        @elseif($member->pivot->status === 'active') bg-green-100 dark:bg-green-900/30
                                        @else bg-red-100 dark:bg-red-900/30 @endif">
                                        <span
                                            class="text-lg font-semibold
                                            @if ($member->pivot->status === 'pending') text-amber-600 dark:text-amber-400
                                            @elseif($member->pivot->status === 'pending_approval') text-orange-600 dark:text-orange-400
                                            @elseif($member->pivot->status === 'active') text-green-600 dark:text-green-400
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <div class="flex flex-wrap gap-2 mt-2">
                                            <span
                                             ...
                                             @elseif($member->pivot->status === 'pending_approval') bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-200
                                             @elseif($member->pivot->status === 'active') bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200
                                             @else bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200 @endif">
                                                @if ($member->pivot->status === 'pending')
                                                    Menunggu
                                                @elseif($member->pivot->status === 'pending_approval')
                                            ...
                                            <span
                                                class="px-3 py-1 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200">
                                ...
                                <div class="flex items-center justify-between sm:justify-end gap-2 sm:gap-3">
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        @if ($canManage && in_array($member->pivot->status, ['pending', 'pending_approval']))
                                            {{-- Accept Button --}}
                                        ...
                                        @if ($canManage && $member->pivot->status === 'active' && $member->id !== $collectiveAction->created_by)
                                            {{-- Remove Button --}}
                                            <button wire:click="removeMember({{ $member->id }})"
                                                onclick="return confirm('Apakah Anda yakin ingin mengeluarkan {{ $member->name }} dari aksi kolektif ini?')"
                                                class="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white rounded text-xs font-medium transition-colors">
                                                Keluarkan
                                            </button>
                                        @endif
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <p class="text-gray-600 dark:text-gray-400">
                            ...
                            @else
                                Belum ada anggota di aksi kolektif ini.
                            @endif
                        </p>
                    </div>
                @endif
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-32 text-center sm:block sm:p-0">
                ...
                <div
                    ...
                    <div class="bg-white dark:bg-slate-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            ...
                            <div class="bg-gray-50 dark:bg-slate-700/50 rounded-lg p-4">
                                <div class="flex items-start space-x-4">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        @if ($selectedMember->profile && $selectedMember->profile->peran)
                                            <p class="text-sm text-gray-600 dark:text-slate-400">
                                        ...
                                        <div class="mt-2 -ml-2">
                                            <span
                                                ...
                                                @elseif($selectedMember->pivot && $selectedMember->pivot->status === 'pending_approval') bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-200
                                                @elseif($selectedMember->pivot && $selectedMember->pivot->status === 'active') bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200
                                                @else bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200 @endif">
                                                @if ($selectedMember->pivot && $selectedMember->pivot->status === 'pending')
                                                    Menunggu
                                                @elseif($selectedMember->pivot && $selectedMember->pivot->status === 'pending_approval')
                                                    Menunggu Persetujuan
                                                @elseif($selectedMember->pivot && $selectedMember->pivot->status === 'active')
                                                    Aktif
                                                @else
                                                    Ditolak
                                                @endif
                        ...
                        <!-- Actions -->
                        @if ($canManage && $selectedMember->pivot && in_array($selectedMember->pivot->status, ['pending', 'pending_approval']))
