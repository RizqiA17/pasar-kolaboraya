    ...
    <div class="bg-gradient-to-r from-primary-blue to-secondary-green text-white rounded-xl p-6">
        <div class="md:block hidden">
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex items-center space-x-3">
                    ...
                    @elseif($collectiveAction->status === 'completed') bg-primary-light-blue/20 dark:bg-primary-blue/30 text-white dark:text-primary-light-blue
                    @else bg-accent-red/20 dark:bg-accent-red/30 text-accent-red @endif">
        ...
        <div class="md:hidden block">
            <div class="flex flex-col gap-4 mb-4">
                ...
                <p class="text-blue-100">{{ $collectiveAction->description }}</p>
                <div class="flex flex-col gap-4 space-x-3">
                    <span
                        class="px-3 py-1 w-fit rounded-full text-sm
                    @if ($collectiveAction->status === 'planning') bg-neutral-orange/20 dark:bg-neutral-orange/30 text-white
                    @elseif($collectiveAction->status === 'active') bg-secondary-green/20 dark:bg-secondary-green/30 text-white
                    @elseif($collectiveAction->status === 'completed') bg-primary-light-blue/20 dark:bg-primary-blue/30 text-white dark:text-primary-light-blue
                    @else bg-accent-red/20 dark:bg-accent-red/30 text-accent-red @endif">
                    ...
                    @if ($collectiveAction->canUserManage(Auth::user()))
                        <a href="{{ route('collective-action.members', $collectiveAction) }}"
                            class="px-4 py-2 w-full bg-white/10 hover:bg-white/20 text-white rounded-lg text-sm font-medium transition-colors">
                            Kelola Anggota
                        </a>
                        <a href="{{ route('collective-action.edit', $collectiveAction) }}"
                            class="px-4 py-2 w-full bg-white/10 hover:bg-white/20 text-white rounded-lg text-sm font-medium transition-colors flex items-center">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z">
                                </path>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
            ...
            <div class="flex flex-wrap gap-3">
                @foreach (['planning' => 'Perencanaan', 'active' => 'Aktif', 'completed' => 'Selesai', 'cancelled' => 'Dibatalkan'] as $status => $label)
                    <button wire:click="updateActionStatus('{{ $status }}')"
                        class="px-4 py-2 rounded-lg text-sm font-medium transition-colors
                            @if ($collectiveAction->status === $status) @if ($status === 'planning') bg-neutral-orange text-white hover:bg-neutral-orange/90
                                ...
                                @else bg-accent-red text-white hover:bg-accent-red/90 @endif
@else
@if ($status === 'planning') bg-neutral-orange/10 text-neutral-orange hover:bg-neutral-orange/20
                                ...
                                @else bg-accent-red/10 text-accent-red hover:bg-accent-red/20 @endif
                            @endif">
                        {{ $label }}
                    </button>
                @endforeach
            ...
            @if ($show_invitation_form)
                <form wire:submit="sendInvitations" class="space-y-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    <div class="relative">
                        ...
                        <!-- Search Input -->
                        <div class="relative">
                            <input type="text" wire:model.live="ecosystem_search"
                                wire:click="$set('show_ecosystem_dropdown', true)" id="ecosystem_search"
                        ...
                        @if ($show_ecosystem_dropdown && count($this->filtered_ecosystems) > 0)
                            <div class="absolute z-50 w-full mt-1 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-60 overflow-y-auto"
                                ...
                                @foreach ($this->filtered_ecosystems as $ecosystem)
                                    <button type="button" wire:click="selectEcosystem({{ $ecosystem->id }})"
                                        data-ecosystem-item
                                        ...
                                        <div class="flex items-start space-x-3">
                                            ...
                                            <div class="flex-shrink-0">
                                                @if (in_array($ecosystem->id, $selected_ecosystem_ids))
                                                    <div
                                                        class="w-5 h-5 bg-purple-600 text-white rounded-full flex items-center justify-center">
                        ...
                        @elseif($show_ecosystem_dropdown && empty($ecosystem_search))
                            <div
                                class="absolute z-50 w-full mt-1 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg p-6">
                                <div class="text-center text-gray-500 dark:text-gray-400">
                                    <div
                                        class="w-12 h-12 mx-auto mb-3 bg-gradient-to-br from-purple-100 to-blue-100 dark:from-purple-900 dark:to-blue-900 rounded-full flex items-center justify-center">
                                    ...
                                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Cari berdasarkan nama,
                                        organisasi, atau wilayah</p>
                                </div>
                            </div>
                        @elseif($show_ecosystem_dropdown && count($this->filtered_ecosystems) == 0)
                            <div
                                ...
                                <div class="text-center text-gray-500 dark:text-gray-400">
                                    <div
                                        class="w-12 h-12 mx-auto mb-3 bg-gradient-to-br from-orange-100 to-red-100 dark:from-orange-900 dark:to-red-900 rounded-full flex items-center justify-center">
                                    ...
                                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">
                                        Tidak ada ekosistem yang cocok dengan "<span
                                            class="font-medium">{{ $ecosystem_search }}</span>"
                                    </p>
                                    <button type="button" wire:click="clearEcosystemSelection"
                        ...
                        @if (count($selected_ecosystems) > 0)
                            <div class="mt-3 space-y-2">
                                <div class="flex items-center justify-between">
                                    ...
                                    <button type="button" wire:click="clearAllSelections"
                                ...
                                <div class="max-h-32 overflow-y-auto space-y-2">
                                    @foreach ($selected_ecosystems as $ecosystem)
                                        <div
                                            class="flex items-center justify-between p-2 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-lg">
                                            <div class="flex items-center space-x-2 flex-1 min-w-0">
                                                <div
                                                    class="flex-shrink-0 w-6 h-6 bg-gradient-to-br from-purple-500 to-blue-500 rounded-md flex items-center justify-center">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <h5
                                                        class="font-semibold text-purple-900 dark:text-purple-200 text-xs truncate">
                                                        {{ $ecosystem->ecosystem_title }}
                                                    </h5>
                        ...
                        @foreach ($selected_ecosystem_ids as $index => $ecosystemId)
                            <input type="hidden" wire:model="selected_ecosystem_ids.{{ $index }}"
                                value="{{ $ecosystemId }}">
                        @endforeach
                    ...
                    <div>
                        ...
                        <textarea wire:model="invitation_message" id="invitation_message" rows="4"
                            placeholder="Tuliskan pesan personal untuk mengundang ekosistem ini berkolaborasi..."
                            class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none"></textarea>
                    ...
                    <div class="flex gap-3">
                        ...
                        <button type="button" wire:click="toggleInvitationForm"
                            class="px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg text-sm font-medium transition-colors">
                            Batal
                        </button>
                    </div>
            ...
            document.addEventListener('livewire:initialized', () => {
                ...
                document.addEventListener('keydown', function(event) {
                    ...
                    let currentIndex = Array.from(items).findIndex(item =>
                        item.classList.contains('bg-gray-100') || item.classList.contains(
                            'ecosystem-item-hover')
                    );
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div
                ...
                <div class="flex items-center justify-between mb-2">
                    ...
                    <span
                        class="text-xl font-bold text-green-600 dark:text-green-400">{{ $aksiQuality['activity_score'] }}%</span>
                </div>
                <div class="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-2 mb-2">
                    <div class="bg-green-500 h-2 rounded-full transition-all duration-1000"
                        style="width: {{ $aksiQuality['activity_score'] }}%"></div>
                </div>
            ...
            <!-- Participation Score -->
            <div
                class="bg-gradient-to-r from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 rounded-lg p-4 border border-orange-200 dark:border-orange-800">
                <div class="flex items-center justify-between mb-2">
                    <h4 class="font-medium text-gray-900 dark:text-slate-100">Tingkat Partisipasi</h4>
                    <span
                        class="text-xl font-bold text-orange-600 dark:text-orange-400">{{ $aksiQuality['participation_score'] }}%</span>
                </div>
                <div class="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-2 mb-2">
                    <div class="bg-orange-500 h-2 rounded-full transition-all duration-1000"
                        style="width: {{ $aksiQuality['participation_score'] }}%"></div>
                </div>
                <p class="text-xs text-gray-600 dark:text-slate-400">
                    {{ $aksiQuality['details']['active_participants'] }} dari
                    {{ $aksiQuality['details']['total_registered'] }} user aktif
                </p>
            </div>

            <!-- Engagement Score -->
            <div
                class="bg-gradient-to-r from-indigo-50 to-indigo-100 dark:from-indigo-900/20 dark:to-indigo-800/20 rounded-lg p-4 border border-indigo-200 dark:border-indigo-800">
                <div class="flex items-center justify-between mb-2">
                    <h4 class="font-medium text-gray-900 dark:text-slate-100">Keterlibatan Ekosistem</h4>
                    <span
                        class="text-xl font-bold text-indigo-600 dark:text-indigo-400">{{ $aksiQuality['engagement_score'] }}%</span>
                </div>
                <div class="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-2 mb-2">
                    <div class="bg-indigo-500 h-2 rounded-full transition-all duration-1000"
                        style="width: {{ $aksiQuality['engagement_score'] }}%"></div>
                </div>
                <p class="text-xs text-gray-600 dark:text-slate-400">
                    {{ $aksiQuality['details']['accepted_ecosystems'] }} dari
                    {{ $aksiQuality['details']['invited_ecosystems'] }} ekosistem
                </p>
            </div>

            <!-- Completion Score -->
            <div
                class="bg-gradient-to-r from-pink-50 to-pink-100 dark:from-pink-900/20 dark:to-pink-800/20 rounded-lg p-4 border border-pink-200 dark:border-pink-800">
                <div class="flex items-center justify-between mb-2">
                    <h4 class="font-medium text-gray-900 dark:text-slate-100">Penyelesaian Kontribusi</h4>
                    <span
                        class="text-xl font-bold text-pink-600 dark:text-pink-400">{{ $aksiQuality['completion_score'] }}%</span>
                </div>
                <div class="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-2 mb-2">
                    <div class="bg-pink-500 h-2 rounded-full transition-all duration-1000"
                        style="width: {{ $aksiQuality['completion_score'] }}%"></div>
                </div>
                <p class="text-xs text-gray-600 dark:text-slate-400">
                    {{ $aksiQuality['details']['completed_contributions'] }} dari
                    {{ $aksiQuality['details']['total_contributions'] }} kontribusi
                </p>
            </div>

            <!-- Diversity Score -->
            <div
                class="bg-gradient-to-r from-teal-50 to-teal-100 dark:from-teal-900/20 dark:to-teal-800/20 rounded-lg p-4 border border-teal-200 dark:border-teal-800">
                <div class="flex items-center justify-between mb-2">
                    <h4 class="font-medium text-gray-900 dark:text-slate-100">Keragaman Kontribusi</h4>
                    <span
                        class="text-xl font-bold text-teal-600 dark:text-teal-400">{{ $aksiQuality['diversity_score'] }}%</span>
                </div>
                <div class="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-2 mb-2">
                    <div class="bg-teal-500 h-2 rounded-full transition-all duration-1000"
                        style="width: {{ $aksiQuality['diversity_score'] }}%"></div>
                </div>
                <p class="text-xs text-gray-600 dark:text-slate-400">
                    {{ $aksiQuality['details']['contribution_types_count'] }} jenis (HHI:
                    {{ $aksiQuality['details']['hhi_value'] }})
                </p>
            </div>
        </div>
                ...
                @if ($collectiveAction->latitude && $collectiveAction->longitude)
                    ...
                    <div class="mt-4">
                        ...
                        <div class="mt-3 text-xs text-gray-500 dark:text-gray-400">
                            <p>Koordinat: {{ number_format($collectiveAction->latitude, 6) }},
                                {{ number_format($collectiveAction->longitude, 6) }}</p>
                        ...
                        <div class="flex items-center p-3 border border-gray-200 dark:border-gray-700 rounded-lg">
                            ...
                            <div class="flex-1">
                                ...
                                <a href="{{ route('ecosystem.dashboard', $ecosystem) }}"
                                    class="text-xs text-primary-blue dark:text-blue-400">
                                    Lihat Ekosistem
                                </a>
                            </div>
                        </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
            ...
            <form wire:submit="submitContribution" class="mb-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <div
                    ...
                    <div class="flex items-center">
                        ...
                        <p class="text-sm text-sky-700 dark:text-blue-300">
                            <strong>Info:</strong> Hanya anggota yang sudah bergabung dengan aksi kolektif yang dapat
                            berkontribusi.
                        </p>
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    @php
                        ...
                        $isFunding =
                            $selectedContribution &&
                            (str_contains(strtolower($selectedContribution->name), 'funding') ||
                                str_contains(strtolower($selectedContribution->name), 'dana'));
                    @endphp
                    @if ($isFunding)
                        <div>
                ...
                <div class="mb-4">
                    <flux:textarea wire:model="contribution_description" :label="'Deskripsi Kontribusi'" required
                        :placeholder="'Jelaskan secara detail kontribusi yang ingin Anda berikan...'" rows="3" />
                </div>
                ...
                <div class="flex gap-3">
                    ...
                    <flux:button type="button" variant="outline" wire:click="toggleContributionForm">
                        Batal
                    </flux:button>
                </div>
            </form>
        ...
        @if ($collectiveAction->canUserManage(Auth::user()) && $pendingContributions->count() > 0)
            <div class="mb-6">
                <h3 class="text-md font-semibold text-gray-900 dark:text-white mb-3">Kontribusi Menunggu Persetujuan
                </h3>
                <div class="space-y-3">
                            ...
                            <div class="flex flex-wrap items-start justify-between gap-4">
                                <div class="flex-1">
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400">
                                        Ditawarkan:
                                        {{ $contribution->offered_at ? $contribution->offered_at->format('d M Y H:i') : 'N/A' }}
                                    </p>
                                </div>
                        ...
                        <div
                            ...
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400">
                                        Diterima:
                                        {{ $contribution->accepted_at ? $contribution->accepted_at->format('d M Y H:i') : 'N/A' }}
                                    </p>
                                </div>
                                @if ($collectiveAction->canUserManage(Auth::user()))
                                    <div class="flex gap-2 ml-4">
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center mb-2">
                                        <h4 class="font-semibold text-gray-900 dark:text-white text-sm">
                                            {{ $contribution->user->name }}</h4>
                                        <span
                                            class="ml-2 px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded-full text-xs">
                                            Selesai
                                        </span>
                                    </div>
                                    <div class="mb-2">
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400">
                                        Selesai:
                                        {{ $contribution->completed_at ? $contribution->completed_at->format('d M Y H:i') : 'N/A' }}
                                    </p>
                                </div>
                            </div>
                        </div>
        ...
        @if (
            $pendingContributions->count() === 0 &&
                $acceptedContributions->count() === 0 &&
                $completedContributions->count() === 0)
            <div class="text-center py-8">
                ...
                @if (
                    !$collectiveAction->canUserContribute(Auth::user()) &&
                        !$collectiveAction->isUserMember(Auth::user()) &&
                        !$collectiveAction->isUserAdmin(Auth::user()) &&
                        !$collectiveAction->isUserContributor(Auth::user()))
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        loadLikeStatus('collective-action', {{ $collectiveAction->id }});
    });
