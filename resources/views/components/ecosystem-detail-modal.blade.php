<div id="ecosystem-detail-modal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4">
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="bg-gray-50 dark:bg-slate-700 p-4 rounded-lg">
                        <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Nama Ekosistem</label>
                        <p id="ecosystem-name" class="text-gray-900 dark:text-slate-100 font-medium"></p>
                    </div>

                    <!-- Organization -->
                    <div class="bg-gray-50 dark:bg-slate-700 p-4 rounded-lg">
                        <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Organisasi</label>
                    ...
                    <div class="bg-gray-50 dark:bg-slate-700 p-4 rounded-lg md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Isu yang Diperjuangkan</label>
                    ...
                    <!-- Work Region -->
                    <div class="bg-gray-50 dark:bg-slate-700 p-4 rounded-lg">
                        <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Wilayah Kerja</label>
                        <p id="ecosystem-work-region" class="text-gray-900 dark:text-slate-100"></p>
                    </div>

                    <!-- Member Count -->
                    <div class="bg-gray-50 dark:bg-slate-700 p-4 rounded-lg">
                        <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Jumlah Anggota</label>
        ...
        <div class="bg-gray-50 dark:bg-slate-700 px-6 py-4 flex justify-end space-x-3">
            <button id="close-ecosystem-modal-btn" class="px-4 py-2 text-gray-600 dark:text-slate-300 bg-gray-200 dark:bg-slate-600 rounded-lg hover:bg-gray-300 dark:hover:bg-slate-500 transition-colors">
                Tutup
            </button>
        </div>
    </div>
    ...
    window.showEcosystemDetails = function(ecosystemData) {
        ...
        if (ecosystemData.ecosystem.issues && ecosystemData.ecosystem.issues.length > 0) {
            ecosystemData.ecosystem.issues.forEach(issue => {
                ...
                issueTag.className = 'inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300';
        ...
        if (ecosystemData.roles && ecosystemData.roles.length > 0) {
            ecosystemData.roles.forEach(role => {
                ...
                roleCard.className = 'bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-4 shadow-sm dark:shadow-slate-900/30';
                roleCard.innerHTML = `
                    ...
                    <p class="text-sm text-gray-600 dark:text-slate-300">${role.description || 'Tidak ada deskripsi tersedia'}</p>
                `;
        ...
        if (ecosystemData.needed_roles && ecosystemData.needed_roles.length > 0) {
            ecosystemData.needed_roles.forEach(role => {
                const roleCard = document.createElement('div');
                roleCard.className = 'bg-orange-50 dark:bg-slate-800 border border-orange-200 dark:border-slate-700 rounded-lg p-4 shadow-sm dark:shadow-slate-900/30';
                roleCard.innerHTML = `
                    <div class="flex items-center justify-between mb-2 ">
            ...
            noNeededRoles.className = 'col-span-2 text-center text-gray-500 dark:text-gray-400 italic py-8';
