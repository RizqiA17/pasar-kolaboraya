<x-admin.layout title="Edit Pengguna - {{ $user->name }}">
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-primary-blue dark:text-secondary-green">Edit Pengguna</h1>
        ...
        <div
            ...
            <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-6">
                ...
                <div class="border-t border-slate-200 dark:border-slate-700 pt-6">
                    <h3 class="text-lg font-semibold text-primary-blue dark:text-secondary-green mb-4">Informasi Tambahan</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <flux:field>
                ...
                <div
                    ...
                    <a href="{{ route('admin.users.show', $user) }}"
                        class="w-full sm:w-auto px-4 py-2 text-gray-600 dark:text-slate-300 hover:text-gray-800 dark:hover:text-slate-200 transition-colors text-center">
                        Batal
                    </a>
                    <button type="submit"
                        class="w-full sm:w-auto px-4 py-2 bg-primary-blue dark:bg-secondary-green text-white rounded-lg hover:bg-sky-800 dark:hover:bg-teal-600 transition-colors">
                        Perbarui Pengguna
                    </button>
                </div>
            </form>
        ...
        @if (!$user->isSuperAdmin() || \App\Models\User::where('role', 'super_admin')->count() > 1)
            <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl p-6">
                <h3 class="text-lg font-semibold text-accent-red dark:text-accent-red-400 mb-4">Zona Bahaya</h3>
                <p class="text-accent-red dark:text-accent-red-400 mb-4">
                ...
                <form method="POST" action="{{ route('admin.users.delete', $user) }}"
                    ...
                    @method('DELETE')
                    <button type="submit"
                        class="w-full sm:w-auto px-4 py-2 bg-accent-red text-white rounded-lg hover:bg-red-700 transition-colors">
                        Hapus Pengguna
                    </button>
                </form>
            </div>
